...
int redir_fcb2unix(cpm_byte *fcb, char *fname)
{
	...
	if (!drv) {
		strcpy(fname, redir_drive_prefix[redir_cpmdrive]);
	} else {
		strcpy(fname, redir_drive_prefix[drv - 1]);
	}
...
#if 0
/* remove high bits - Hitech C is very strange */
scrub_fcb(cpm_byte *fcb)
{
	unsigned char c;
	char buffer[20];
	char nbuffer[20];
	...
	strncpy(buffer, fcb+1, 11);
	buffer[12] = 0;

	for (i = 1;i != 12; i++) { /* walk filename/ext */
		c = *(fcb+i);
		if (c > 0x80) {
			*(fcb+i) = c & 0x7f;
		}
	}
	strncpy(nbuffer, fcb+1, 11);
}
#endif
