...
int fcb_init(void)
{
	...
	/* E: to O: free - A-D taken (MAXALT) */
	for (n = maxalt; n < 15; n++) redir_drive_prefix[n][0] = 0;

	/* initialize drives after alternate paths are set */
	strcpy(redir_drive_prefix[0], "./");	/* current drive */
	strcpy(redir_drive_prefix[1], "./");	/* drive A: */
	for (count = 2; count != maxalt; count++) {
		strcpy(redir_drive_prefix[count],altfiles[count]);	/* b: */
		strcpy(redir_drive_prefix[count],altfiles[count]);	/* c: */
		strcpy(redir_drive_prefix[count],altfiles[count]);	/* d: */
	}

	/* Log on to A:. It is the only drive at this point which we
     * know works.
	 */
	redir_cpmdrive = 1;
#ifdef __MSDOS__
 ...
 *
 * CP/M drives A: to O: can be mapped in this way. A: is always the current
 * drive. 
 *
 */
...
void xlt_name(char *localname, char *cpmname)
{
	...
	if (!pname)	/* No path separators in the name. It is therefore a
                           local filename, so map it to drive A: */
	{
		strcpy(cpmname, "A:");
		strcat(cpmname, ibuf);
		return;
...
int xlt_map(int drive, char *localdir)
{
	...
	strcpy(redir_drive_prefix[0], "./");	/* current drive */
	strcpy(redir_drive_prefix[1], "./");	/* drive A: */
	/* set needed permanent drive mappings */
	for (count = 2; count != maxalt; count++) { /* drives B,C,D */
		strcpy(redir_drive_prefix[count], altfiles[count]);
printf("loaded drive_prefix %d with %s\n", count, altfiles[count]);
	}
	if (drive == -1) {
		/* 1st four are taken by altfiles[0-4] (MAXALT) */
		for (n = maxalt; n < 15; n++) {
			if (!redir_drive_prefix[n][0]) {
				drive = n;
				break;
