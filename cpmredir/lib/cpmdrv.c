...
{
	if (drv > 16) {
		/* Exceeds maximum CP/M drive number and SLR Systems
		 * software does, so return error
		 *
		 * In the Windows version, no select is allowed
		 * except 0 or 1 anyway.  No writes are permitted
		 * on any drive except 0 or 1 i.e. current drive
		 * or A:
		 */
		return 0x4FF; /* Drive doesn't (can't) exist */
	}
	if (redir_drive_prefix[drv][0]) {
		...
		return 0x04FF;	/* Drive doesn't exist */
	}
}
