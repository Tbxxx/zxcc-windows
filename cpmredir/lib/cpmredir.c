...
/* Opening some other directory */
...
#if 0
	if (strncmp(progname, "ht", 2) == 0) {
		/* clean up high bits in fcb for HiTech */
		scrub_fcb(fcb);
	}
#endif
	...
	/* store handles of HiTech C temporary files */
	if (strstr(fname, "CTMP1")) {
		h1 = handle;
	} else if (strstr(fname, "CTMP2")) {
		h2 = handle;
	} else if (strstr(fname, "CTMP3")) {
		h3 = handle;
	} else if (strstr(fname, "CTMP4")) {
		h4 = handle;
	}
...
cpm_word fcb_close(cpm_byte *fcb)
{
	...
	SHOWNAME("fcb_close")
        ...
        if ((handle = redir_verify_fcb(fcb)) < 0) {
			return -1;
		}
...
cpm_word fcb_creat(cpm_byte *fcb, cpm_byte *dma)
{
        char fname[CPM_MAXPATH];
        int handle;
		char basefname[CPM_MAXPATH];
		char tmpname[CPM_MAXPATH];
		char *basename(char *str);
		int rc;
		int seq = '1';
		char buffer[CPM_MAXPATH];
		int ht = 0;		/* is the program part of HiTech C? */
...
*/
		/* easy on HT perms */
		if (strncmp(progname, "ht", 2) == 0) {
        	handle = open(fname, O_RDWR | O_CREAT | O_BINARY, 
                S_IREAD | S_IWRITE);
		} else {
        	handle = open(fname, O_RDWR | O_CREAT | O_EXCL | O_BINARY, 
                S_IREAD | S_IWRITE);
		...
		/* HiTech C hack */
        if (handle < 0 ) {
			/* close handles of HiTech C temporary files before
			 * opening them again.  HiTech reuses file names.  Ugh...
			 */
			if (strstr(fname, "CTMP1") || strstr(fname, "ctmp1")) {
				rc = close(h1);
				rc = unlink(fname);
				ht = 1;
			} else if (strstr(fname, "CTMP2") || strstr(fname, "ctmp2")) {
				rc = close(h2);
				rc = unlink(fname);
				ht = 1;
			} else if (strstr(fname, "CTMP3") || strstr(fname, "ctmp3")) {
				rc = close(h3);
				rc = unlink(fname);
				ht = 1;
			} else if (strstr(fname, "CTMP4") || strstr(fname, "ctmp4")) {
				rc = close(h4);
				rc = unlink(fname);
				ht = 1;
			}
			...
			/* for HiTech only */
			if (ht == 1) {
				/* retry open for creat now that's it's properly closed */
       			 handle = open(fname, O_RDWR | O_CREAT |O_BINARY, 
                             S_IREAD | S_IWRITE);

/*
		printf("HiTech retry open: handle = %d, errno = %d\n", handle, errno);
		printf("for filename: %s\n", fname);
*/
				if (handle < 0) {
					/* It really will not open - return failure */
					return 0xff;
				}
			} else {
				/* return the original failure - not HiTech C related */
				return(0xff);
			}
		}
		/* open OK, proceed onwards... */
